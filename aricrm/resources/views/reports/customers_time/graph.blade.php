    ...
    window.addEventListener('load', () => {
      ...
      const myConfig = {
        ...
        scaleX: {
          ...
          labels: [@foreach($time_array as $time) '{{$time[0]}}' @if($i <(count($time_array)-1)) , @endif @endforeach]
          ,
        },
        scaleY: {
        ...
        series: [
          ...
          
          {
            ...
            values: [@for($j=0;$j< count($time_array); $j++){{$data[$j][$i]}}@if($j<(count($time_array)-1)) , @endif @endfor],
            text: '{{$customer_statuses[$i]->name}}',
            backgroundColor: colors[{{$i}}]
          }
          @if($i<(count($customer_statuses)-1)) , @endif 
          @endfor
        ]
      };
