...
class Socrata {
  ...
  private $root_url = "https://www.datos.gov.co";
...
public function getTRM(){
        ...
        $response = $this->getSocrataData($params);
        
        return $response[0]['valor'];   
    }

    public function getSocrataData($params){
        $data = array(
            'APP_TOKEN' => 'nz4Uxz36kLGXgnmuoV3u861ha',
            'SOCRATA_USER' => 'nicolas@myseocompany.co',
            'SOCRATA_PASSWORD' => 'Nico&1014'
        );
        try {

            $client = new Socrata("www.datos.gov.co/", $data["APP_TOKEN"], $data["SOCRATA_USER"], $data["SOCRATA_PASSWORD"]);
            
            $response = $client->get("resource/ceyp-9c7c.json", $params);
        }catch (Exception $e) {
            $data = array();
            $response = $data;
        }
